...
import { PiggyBank, Plus, Trash2, ArrowUpCircle, ArrowDownCircle, Pencil } from 'lucide-react';
        ...
        <div className={`${pieData.length > 0 ? 'lg:col-span-2' : 'lg:col-span-3'} space-y-3 animate-in-delay-2`}>
          {savingsGoals.length === 0 && (
            <div className="glass rounded-3xl p-8 text-center space-y-3">
              <PiggyBank className="w-12 h-12 text-muted-foreground mx-auto" />
              <p className="text-foreground font-bold">Crie sua primeira meta</p>
              <p className="text-muted-foreground text-sm max-w-sm mx-auto">
                O Pé de Meia permite guardar dinheiro para objetivos como viagens, emergências ou compras. Crie metas e acompanhe o progresso!
              </p>
            </div>
          )}
              ...
              <div key={goal.id} className="glass rounded-3xl p-5 space-y-3 group">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground text-xs">Meta: {fmt(target)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setShowAddTx(goal.id)} className="p-2 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
                      <Plus className="w-4 h-4" />
                    </button>
                    <button onClick={() => setEditGoal(goal)} className="opacity-0 group-hover:opacity-100 p-2 text-muted-foreground hover:text-foreground transition-all">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => deleteSavingsGoal(goal.id)} className="opacity-0 group-hover:opacity-100 p-2 text-muted-foreground hover:text-expense transition-all">
                ...
                <div>
                  ...
                  <div className="w-full bg-muted rounded-full h-2.5">
                    <div className="h-2.5 rounded-full gradient-income transition-all" style={{ width: `${pct}%` }} />
                  </div>
                </div>
                  ...
                  <div className="space-y-1 pt-2 border-t border-border/50">
                    {goalTxs.slice(0, 3).map((t) => (
                      <div key={t.id} className="flex items-center justify-between text-sm group/tx">
                        <div className="flex items-center gap-2">
                          {t.type === 'DEPOSIT' ? <ArrowUpCircle className="w-4 h-4 text-income" /> : <ArrowDownCircle className="w-4 h-4 text-expense" />}
                          <span className="text-muted-foreground text-xs">{format(parseISO(t.date), 'dd/MM/yyyy', { locale: ptBR })}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`font-semibold ${t.type === 'DEPOSIT' ? 'text-income' : 'text-expense'}`}>
                            {t.type === 'DEPOSIT' ? '+' : '-'}{fmt(Number(t.amount))}
                          </span>
                          <button onClick={() => deleteSavingsTransaction(t.id)} className="opacity-0 group-hover/tx:opacity-100 text-muted-foreground hover:text-expense">
